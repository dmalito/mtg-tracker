<script>
  ...
  async function save() {
    ...
    try {
      ...
      const winner = form.draw ? '' : (hasScores ? autoWinner : form.winner);
...
<div class="fade-in">
  <div class="card">
    ...
    <div class="fg">
      ...
      <div class="pill-row">
        <button class="pill" class:active-pau={!form.draw} on:click={() => form.draw = false}>
          Has Winner
        </button>
        <button class="pill" class:active-cmd={form.draw} on:click={() => form.draw = true}>
          Draw
        </button>
      </div>
    </div>

    <!-- Winner — only shown if not a draw -->
    {#if !form.draw}
      <div class="fg">
