  ...
  async function saveEdit(id) {
    ...
    try {
      const updated = await api.updateGame({
        ...
        winner:  editForm.draw ? '' : (editAutoWinner || ''),
        draw:    editForm.draw,
        ...
        {#if editingId === g.id && editForm}
          <div class="edit-form fade-in">
            ...
            <div class="fg">
              ...
              <div class="pill-row">
                <button class="pill" class:active-pau={!editForm.draw} on:click={() => editForm.draw = false}>
                  Has Winner
                </button>
                <button class="pill" class:active-cmd={editForm.draw} on:click={() => editForm.draw = true}>
                  Draw
                </button>
              </div>
            </div>
            ...
            {#if editForm.draw}
              <div class="auto-winner draw-notice" style="margin-bottom:.85rem">
                🤝 Draw — ELO adjusted for all players
              </div>
            {:else if editAutoWinner}
              <div class="auto-winner" style="margin-top:.5rem;margin-bottom:.85rem">🏆 {editAutoWinner}</div>
            {/if}
